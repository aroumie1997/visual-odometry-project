...
if ds == 0
    ...
    [S0, S1, Twc1] = distTriangulation_Nuria(img0, img1, K);
...
end% Makes sure that plots refresh.    
pause(0.01);
...
for i = range
    ...
    % Localize
    [loc_keyframe_S, loc_S, loc_Twc] = ransacLocalization_Nuria(keyframe_image, image, keyframe_S.keypoints, keyframe_S.landmarks, K);

    num_loc_matches = size(loc_S.keypoints, 2);
    
    if size(loc_S.keypoints,2)<min_keypoints
        fprintf('Adding new landmarks\n=====================\n');
        [loc_keyframe_S, loc_S, loc_Twc] = distTriangulation_Nuria_renew(keyframe_image, image,loc_Twc,K);
        num_loc_matches = size(loc_S.keypoints, 2);
    end
...
%     figure(3)
%     showMatchedFeatures(keyframe_image,image,loc_keyframe_S.keypoints',loc_S.keypoints','montage','PlotOptions',{'ro','go','y--'});
%     title('Matches Localization');
    ...
    hold on;
    matches = 1 : size(loc_S.keypoints, 2);
    ...
    keyframe_S.keypoints=loc_keyframe_S.keypoints;
    keyframe_S.landmarks=loc_S.landmarks;
